...
LOG_REGEX = [
    ...
    r'^(?P<event>Kill): (?P<actor_id>\d+) (?P<target_id>\d+) (?P<weapon_id>\d+): (?P<actor_name><?.+?>?) killed (?P<target_name>.+?) by (?P<weapon_name>\w+)$',
    r'^(?P<event>Exit): (?P<msg>.*)$',
    r'^(?P<event>tell): (?P<actor_name>.+?) to (?P<target_name>.+?): (?P<msg>.*)$',
    r'^(?P<event>sayteam): (?P<actor_name>.+?): (?P<msg>.*)$',
    r'^(?P<event>say): (?P<actor_name>.+?): (?P<msg>.*)$',  ## TODO: match names like ^1A^2n^3a^4r^5k^6i
    r'^(?P<event>voice): (?P<actor_name>.+?) (?P<voice>.*)$',
]
